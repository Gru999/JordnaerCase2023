...
namespace JordnærCase2023.Services
{
    public class EventService : Connection, IEventService
    {
        ...
        private string queryGetAllMembersForEvent = "select Member.Member_ID, Member.Member_Name, Member.Member_Img, Member.Member_Phone, Member.Member_Email, Member.Member_Password, Member.Member_SanitationCourse, Member.Member_Admin from Member left join EventMember on Member.Member_ID = EventMember.Member_ID AND EventMember.Event_ID = @EventId";
        private string queryCreateEventMember = "insert into EventMember values (@MemberId, @EventId)";
        private string queryDeleteEventMember = "delete from EventMember where Member_ID = @MemberId and Event_ID = @EventId";
        private string queryEMIdFromId = "select * from EventMember";
        private string queryDeleteEMOwnerAdmin = "delete from EventMember where Event_ID = @EventId"; 
        ...
        public async Task<List<Member>> GetMembersFromEventAsync(int eventId)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    commmand.Parameters.AddWithValue("@EventId", eventId);
                    await commmand.Connection.OpenAsync();
                    SqlDataReader reader = await commmand.ExecuteReaderAsync();
                    while (await reader.ReadAsync())
                    {
        ...
        public async Task<List<Tuple<int, int, int>>> GetAllEventMemberAsync()
        {
            List<Tuple<int, int, int>> eventList = new List<Tuple<int, int, int>>();
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
        ...
        public async Task<Event> DeleteEMAsync(int memberId, int eventId)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
        ...
        public async Task<Event> DeleteEMOwnerAdminAsync(int eventId)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    SqlCommand command = new SqlCommand(queryDeleteEMOwnerAdmin, connection);
                    command.Parameters.AddWithValue("@EventId", eventId);
                    await command.Connection.OpenAsync();
                    SqlDataReader reader = await command.ExecuteReaderAsync();
                    if (await reader.ReadAsync())
